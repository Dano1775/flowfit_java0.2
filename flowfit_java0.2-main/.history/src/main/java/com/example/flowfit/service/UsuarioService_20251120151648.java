...
public class UsuarioService {
    ...
    public LoginResult login(String correo, String clave) {
        try {
            // Buscar usuario por correo electrónico (eliminar espacios)
            Optional<Usuario> usuarioOpt = usuarioRepository.findByCorreo(correo.trim());
            
            // Usuario no encontrado o contraseña incorrecta
            if (usuarioOpt.isEmpty() || !usuarioOpt.get().getClave().equals(clave.trim())) {
     ...
     */
    public List<Usuario> obtenerUsuariosPorEntrenador(Integer entrenadorId) {
        List<AsignacionEntrenador> asignaciones = asignacionEntrenadorRepository
            .findByEntrenadorIdAndEstado(entrenadorId, AsignacionEntrenador.EstadoAsignacion.Activa);
        
        return asignaciones.stream()
            .map(asignacion -> usuarioRepository.findById(asignacion.getUsuarioId()).orElse(null))
