    ...
    public ResponseEntity<Map<String, Object>> obtenerDetallesRutinaApi(@RequestParam Integer rutinaId,
        ...
        try {
            ...
            if (usuario == null) {
                ...
                // Usar usuario de prueba para testing
                Optional<Usuario> usuarioOpt = usuarioService.buscarPorCorreo("juan.perez@test.com");
                if (usuarioOpt.isPresent()) {
                    usuario = usuarioOpt.get();
                    System.out.println("Usuario de prueba cargado: " + usuario.getNombre());
                } else {
                    // Usar primer usuario disponible como fallback
                    List<Usuario> todosUsuarios = usuarioService.obtenerTodosLosUsuarios();
                    if (!todosUsuarios.isEmpty()) {
                        usuario = todosUsuarios.get(0);
                        System.out.println("Usando primer usuario disponible: " + usuario.getNombre());
                    } else {
