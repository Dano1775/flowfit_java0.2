...
public class AdminController {
    ...
    public String adminDashboard(Model model, HttpSession session) {
        ...
        // Obtener actividad reciente (últimos 10 registros de aprobación)
    ...
    public String usuarios(@RequestParam(defaultValue = "0") int page,
        ...
        try {
            ...
            for (Usuario u : todosUsuarios) {
                System.out.println("- " + u.getId() + ": " + u.getNombre() + " (" + u.getPerfilUsuario() + ")");
            }
            
            // Aplicar filtro de búsqueda
            if (!search.trim().isEmpty()) {
                System.out.println("Aplicando filtro de búsqueda: '" + search + "'");
                todosUsuarios = todosUsuarios.stream()
                    ...
                    .collect(java.util.stream.Collectors.toList());
                System.out.println("Usuarios después del filtro de búsqueda: " + todosUsuarios.size());
            }
            
            // Aplicar filtro de estado
            if (!"todos".equals(filtro)) {
                System.out.println("Aplicando filtro de estado: '" + filtro + "'");
                todosUsuarios = todosUsuarios.stream()
                    .filter(u -> {
                        switch(filtro) {
                    ...
                    .collect(java.util.stream.Collectors.toList());
                System.out.println("Usuarios después del filtro de estado: " + todosUsuarios.size());
            }
            ...
            System.out.println("FINAL - Enviando al modelo: " + todosUsuarios.size() + " usuarios");
            System.out.println("Filtro: " + filtro + ", Search: '" + search + "'");
    ...
    public String rechazarUsuario(@RequestParam("id") Integer usuarioId, 
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("success", "Usuario rechazado correctamente.");
    ...
    public String eliminarUsuario(@PathVariable Integer id, 
        ...
        try {
            Usuario admin = (Usuario) session.getAttribute("usuario");
            if (admin == null || !Usuario.PerfilUsuario.Administrador.equals(admin.getPerfilUsuario())) {
            ...
            if (Usuario.PerfilUsuario.Administrador.equals(usuarioAEliminar.getPerfilUsuario())) {
            ...
            String perfilUsuario = usuarioAEliminar.getPerfilUsuario().toString();
            
            usuarioService.eliminarUsuario(id);
            
            // Registrar la eliminación en el historial
            HistorialEdiciones historial = new HistorialEdiciones();
            historial.setUsuarioEditado(usuarioAEliminar);
            historial.setAdminEditor(admin);
            historial.setFechaEdicion(LocalDateTime.now());
            historial.setCampoEditado("ELIMINACION_USUARIO");
            historial.setValorAnterior("Usuario activo - Nombre: " + nombreUsuario + ", Perfil: " + perfilUsuario);
            historial.setValorNuevo("USUARIO ELIMINADO");
            historial.setMotivoCambio("Usuario eliminado por administrador");
            historialEdicionesRepository.save(historial);
            
            redirectAttributes.addFlashAttribute("success", 
                "Usuario '" + nombreUsuario + "' eliminado exitosamente del sistema");
            return "redirect:/admin/usuarios";
