...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initializeAssignedRoutines() {
        // Check if test user has assigned routines
        long assignedCount = rutinaAsignadaRepository.count();
        System.out.println("Current assigned routines count: " + assignedCount);
        ...
        if (assignedCount == 0) {
            ...
            System.out.println("Assigned routines already exist, listing existing assignments:");
            List<RutinaAsignada> existing = rutinaAsignadaRepository.findAll();
            for (RutinaAsignada ra : existing) {
                System.out.println("- User " + ra.getUsuarioId() + " -> Rutina " + ra.getRutinaId() + " (Progress: " + ra.getProgreso() + "%, Status: " + ra.getEstado() + ")");
            }
        }
    }
