...
public class ExcelExportService {
     ...
     */
    public ByteArrayOutputStream generateHistorialAprobacionesExcel(List<RegistroAprobaciones> historial) throws IOException {
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            addStatisticRow(sheet, rowNum++, "Total de usuarios rechazados:", String.valueOf(totalRechazados), dataStyle);
            ...
            String[] headers = {"Fecha", "Hora", "Usuario", "Documento", "Correo Electrónico", "Perfil", "Acción", "Detalles", "Administrador"};
            ...
            for (RegistroAprobaciones registro : historial) {
                ...
                if ("Aprobado".equals(registro.getAccion().toString())) {
                    ...
                    actionCell.setCellStyle(dataStyle); // Use normal style for edited
                } else {
    ...
    public ByteArrayOutputStream generateUsuariosExcel(List<Usuario> usuarios, String search, String filtro) throws IOException {
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            String[] headers = {"ID", "Nombre Completo", "Email", "Teléfono", "Documento", "Perfil", "Estado", "Fecha Registro", "Fecha Último Acceso"};
            for (int i = 0; i < headers.length; i++) {
                Cell cell = headerRow.createCell(i);
                cell.setCellValue(headers[i]);
                cell.setCellStyle(headerStyle);
            ...
            for (Usuario usuario : usuarios) {
                ...
                profileCell.setCellValue(usuario.getPerfilUsuario() != null ? usuario.getPerfilUsuario().toString() : "Usuario");
                profileCell.setCellStyle(dataStyle);
                ...
                // Fecha Último Acceso (placeholder)
    ...
    private void createUsersSummarySheet(Sheet sheet, List<Usuario> usuarios, Workbook workbook, String search, String filtro) {
        CellStyle titleStyle = createTitleStyle(workbook);
        CellStyle headerStyle = createHeaderStyle(workbook);
        CellStyle dataStyle = createDataStyle(workbook);
        ...
        infoCell1.setCellValue("Fecha de generación: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss")));
        ...
        long nutricionistas = usuarios.stream().filter(u -> u.getPerfilUsuario().toString().equals("Nutricionista")).count();
        long usuariosNormales = usuarios.stream().filter(u -> u.getPerfilUsuario().toString().equals("Usuario")).count();
        long administradores = usuarios.stream().filter(u -> u.getPerfilUsuario().toString().equals("Administrador")).count();
