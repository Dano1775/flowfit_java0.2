...
function descargarRutinaPDF(rutinaId, rutina, ejercicios) {
  ...
  ejercicios.forEach((ej, idx) => {
    ...
    let imagenSrc = '';
    if (ej.ejercicioImagen) {
      if (ej.ejercicioImagen.startsWith('http://') || ej.ejercicioImagen.startsWith('https://')) {
        imagenSrc = ej.ejercicioImagen;
      } else {
        imagenSrc = '/ejercicio_image_uploads/' + ej.ejercicioImagen;
      }
    }
    ...
    contenidoPDF += `
      <div class="ejercicio">
        ...
        ${imagenSrc ? '<div style="text-align: center; margin: 15px 0;"><img src="' + imagenSrc + '" alt="' + nombre + '" style="max-width: 400px; max-height: 300px; border-radius: 8px; border: 2px solid #e5e7eb;"></div>' : ''}
